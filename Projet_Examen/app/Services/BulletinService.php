...
class BulletinService
{
    ...
    public function genererBulletin(int $eleveId, string $periode, string $anneeScolaire = null): Bulletin
    {
        return DB::transaction(function () use ($eleveId, $periode, $anneeScolaire) {
            ...
            $notificationService->envoyerNotificationBulletin($bulletin);
    ...
    private function genererPDFBulletin(Bulletin $bulletin, array $moyennesParMatiere): string
    {
        ...
        $data = [
            ...
            'periodeFormatee' => $this->formaterPeriode($bulletin->periode)
        ];

        $pdf = Pdf::loadView('pdfs.bulletin', $data);
        
        $filename = 'bulletin_' . $eleve->id . '_' . $bulletin->periode . '_' . date('Y-m-d') . '.pdf';
     ...
     */
    public function getAllBulletinsWithRelations()
    {
        return Bulletin::with(['eleve.user', 'classe'])->orderBy('date_edition', 'desc')->get();
    }

    /**
     * Récupérer un bulletin par son ID avec les relations nécessaires
     */
    public function getBulletinByIdWithRelations($id)
    {
        return Bulletin::with(['eleve.user', 'classe'])->find($id);
    }

    /**
     * Mettre à jour un bulletin
     */
    public function updateBulletin($id, array $data)
    {
        ...
        $bulletin->update($validator->validated());
        return $bulletin;
    }
